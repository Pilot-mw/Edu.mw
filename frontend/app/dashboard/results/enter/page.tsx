...
import api, { ClassSubject, Student, Exam, ClassRoom } from '@/app/services/api';
...
export default function AdminEnterMarksPage() {
  ...
  const fetchClassRooms = async () => {
    try {
      ...
      console.error('Failed to fetch class rooms:', error);
    }
  };
  ...
  const handleClassRoomChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (classRoomId) {
      const filtered = classSubjects.filter(
        (cs) => cs.class_room.toString() === classRoomId
      );
  ...
  const handleSubjectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (subjectId && selectedClassRoom) {
      const cs = classSubjects.find(
        (item) =>
          item.class_room.toString() === selectedClassRoom &&
          item.subject.toString() === subjectId
      );
      if (cs) {
        setSelectedClassSubject(cs.id.toString());
        fetchStudentsAndMarks(cs);
      }
    } else {
      setStudents([]);
      setMarks({});
  ...
  const fetchStudentsAndMarks = async (cs: ClassSubject) => {
    try {
